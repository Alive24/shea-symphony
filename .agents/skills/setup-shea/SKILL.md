...
3. Read [workflow-project.md](references/workflow-project.md) when binding a
   workflow, capability adapter, App profile, or GitHub Project.
4. Read [reconciliation.md](references/reconciliation.md) to plan selected
...
5. Read [runtime-profile.md](references/runtime-profile.md) when runtime
   requirements are missing, stale, drifted, or explicitly requested.
6. Read [readiness.md](references/readiness.md) for readback, verification, and
...
- Use the standard Skills CLI's project-local `--copy` installation in a
  ...
  files into the target. Do not create a Shea package manager, suite manifest,
  package database, or upstream-hash registry.
