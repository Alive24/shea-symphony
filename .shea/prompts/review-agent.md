...
- `agy`-backed `review loop` runs headlessly with `agy --print --mode plan`; `codex-app-server` review runs a fresh read-only, non-interactive thread; `claude-code` review runs a fresh stream-json session through the shared Claude transport. All are report-only and preserve durable protocol/stdout/stderr/job-ledger evidence; the Shea Symphony CLI wrapper records evidence and changes state after the backend exits.
