## Shea Symphony Parent-Batch Readiness Report

This readiness report is read-only and advisory. It prepares the operator for UAT on a native parent issue, but it is not a Human Review decision note, does not record approval, and must not write tracker comments, Project state, PR state, issue bodies, workpads, or child evidence.
...
For a parent issue shaped like #400, the report should be able to show remaining parent UAT, parent PR #421 readiness, child #399/#383/#384 status and child PR merge evidence, parent Review PASS evidence, and any preserved risks without requiring the operator to open every child issue before UAT starts.
