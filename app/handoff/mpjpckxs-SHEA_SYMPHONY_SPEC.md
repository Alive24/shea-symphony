# Shea Symphony Specification

Status: Bootstrap v0

## Purpose

Shea Symphony is a private-first team harness for orchestrating coding agents
against tracked engineering work. It is a Rust implementation of an
OpenAI-Symphony-style orchestration system with Shea Symphony-specific workflow
extensions.
...
Shea Symphony should preserve the capability categories present in the official
...
5. Shea Symphony-specific bootstrap docs in `docs/bootstrap/`

`SPEC.md` is the normative protocol baseline. The Elixir implementation is the
feature-parity baseline. Shea Symphony may use different Rust architecture, but
...
Shea Symphony does not need to copy Elixir module structure or Phoenix
...
## Shea Symphony Extensions

Shea Symphony adds these workflow capabilities on top of the official baseline:
...
Shea Symphony should operate as a continuing tracker loop, not a one-issue
...
Shea Symphony should normalize tracker states to:
...
Shea Symphony must distinguish the main implementation agent from the
...
Shea Symphony should preserve the official observability capability category.
