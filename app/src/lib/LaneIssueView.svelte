<script lang="ts">
  ...
  import { completedProgressDisplay } from "./viewModel/completedWorktrees.ts";
  import { buildIssueCommentLifecycleEvents } from "./viewModel/githubIssueTimeline.ts";
  import {
    issueIdentityTitle,
  ...
  export let transcriptFixture: any = null;
  ...
  let transcriptError = "";
  let transcriptResponse: any = transcriptUnavailable(
    "Transcript has not been loaded yet.",
  );
  ...
  $: titledCompletedLocalIssues = completedLocalIssues.filter(
    hasCompleteIssueTitle,
  );
  $: filteredCompletedLocalIssues = filterCompletedByWindow(
    titledCompletedLocalIssues,
    completedWindowHours,
  );
  $: completedPageCount = Math.max(
    1,
    Math.ceil(filteredCompletedLocalIssues.length / completedPageSize),
  );
  $: completedPage = Math.min(completedPage, completedPageCount);
  $: pagedCompletedLocalIssues = filteredCompletedLocalIssues.slice(
    (completedPage - 1) * completedPageSize,
    completedPage * completedPageSize,
  );
  $: completedPageStart = filteredCompletedLocalIssues.length
    ? (completedPage - 1) * completedPageSize + 1
    : 0;
  $: completedPageEnd = Math.min(
    completedPage * completedPageSize,
    filteredCompletedLocalIssues.length,
  );
  ...
  $: selectedIssue = selectedIssueRef
    ? findIssueForDetail(
        selectedIssueRef,
        issueRows,
        completedLocalIssues,
        view,
      )
    : null;
  $: localArtifactsRefresh = $operatorOverviewStore.localArtifactsRefresh;
  $: remoteLifecycleEvents = selectedIssue
    ? buildIssueCommentLifecycleEvents(issueTimelineResponse, selectedIssue)
    : [];
  $: lifecycleEvents = selectedIssue
    ? buildLifecycleEvents(selectedIssue, view, remoteLifecycleEvents)
    : [];
  $: selectedLaneKey = laneKeyForIssue(selectedIssue);
  $: heartbeatSummary = selectedIssue
    ? classifyHeartbeat(
        $autoloopStateStore,
        selectedLaneKey,
        selectedIssue.id,
  ...
  $: laneColumns = laneOrder.map((lane) => {
    const issues =
      lane === "Human Todo"
        ? issueRows.filter((issue) =>
            humanStates.has(normalizeState(issue.state)),
          )
        : issueRows.filter(
            (issue) =>
              issue.lane === lane &&
    ...
    return {
      lane,
      issues,
      pickedCount: issues.filter(
        (issue) => issue.runtimeCategory === "Active runtime",
      ).length,
      completedCount: issues.filter(
        (issue) => issue.runtimeCategory === "Recent completion",
      ).length,
    };
  });
  ...
  function buildIssueRows(
    issues: any[] = [],
    workersByLane: Record<string, any[]> = {},
    model: any = {},
  ) {
    const byId = new Map();

    for (const issue of issues ?? []) {
      const id = normalizeIssueRef(
        issue.id ?? issue.identifier ?? issue.number,
      );
      if (!id) continue;
    ...
    for (const [laneKey, workers] of Object.entries(workersByLane ?? {})) {
      for (const worker of workers as any[]) {
        ...
        byId.set(id, {
          ...existing,
          lane: humanStates.has(normalizeState(existing.state))
            ? "Human Todo"
            : existing.lane || titleCase(laneKey),
          title: issueIdentityTitle(
            id,
            [existing.title, worker.title],
            "Issue",
          ),
          worker,
  ...
  function baseIssueRow(issue: any, model: any) {
    ...
    const id =
      normalizeIssueRef(issue.id ?? issue.identifier ?? issue.number) ??
      issue.id;
    return {
      id,
      number: issue.number ?? issueNumber(id),
      title: issueIdentityTitle(id, [issue.title], "Issue"),
      lane: humanStates.has(normalizeState(state))
        ? "Human Todo"
        : (issue.lane ?? stateToLane(state)),
      state,
      ...
      runtimeCategory: completed
        ? "Recent completion"
        : runtimeCategoryForIssue(issue),
      runtimeTone: completed
        ? "success"
        : issue.workerStatus === "Worker read unavailable"
          ? "warn"
          : humanStates.has(normalizeState(state))
            ? "warn"
            : "neutral",
      runtimeDetail: completed
        ? "Local runtime is no longer active; use the issue timeline for closeout evidence."
        : (issue.workerDetail ??
          "No active local runtime is visible for this issue."),
  ...
  function buildCompletedLocalIssues(model: any, rows: any[]) {
    ...
    for (const entry of model?.raw?.localStatus?.issueWorktrees ?? []) {
      ...
      if (existing) {
        byId.set(id, {
          ...existing,
          worktree: { ...existing.worktree, ...entry },
        });
        ...
        byId.set(
          ...
          normalizeCompletedEntry(
            { ...entry, state: row?.state, completedAt: entry.lastModified },
            rows,
            model,
          ),
        );
    ...
    return [...byId.values()].sort(
      (left, right) => completedLocalSortMs(right) - completedLocalSortMs(left),
    );
  }
  ...
  function normalizeCompletedEntry(entry: any, rows: any[], model: any) {
    ...
    const lastProgressSource =
      entry.lastProgressSource ??
    ...
    return {
      ...
      worktree: {
        ...
        lastModified,
        lastModifiedSource:
          entry.lastModifiedSource ?? timestampSources?.lastModified?.source,
        timestampSources,
  ...
  function buildLifecycleEvents(
    ...
    const fixtureEvents = (
      model?.raw?.localStatus?.issueLifecycle?.[issue.id] ??
      model?.raw?.localStatus?.issueLifecycle?.[issue.id?.replace("#", "")] ??
      []
    ).map((event: any) => ({
      label: event.label ?? event.phase ?? "Issue timeline event",
    ...
    const baseEvents = fixtureEvents.length
      ? fixtureEvents
      : inferLifecycleEvents(issue, model, remoteEvents);
    const events = [...baseEvents, ...(remoteEvents ?? [])];
    return dedupeEvents(events).sort(
      (left, right) => eventSortMs(right) - eventSortMs(left),
    );
  }
  ...
  function laneEventEvidence(issue: any, model: any, remoteEvents: any[] = []) {
    ...
    const persistedEvents =
      localLifecycle?.[issue?.id] ??
      localLifecycle?.[issue?.id?.replace("#", "")] ??
      [];
  ...
  function eventMentionsIssue(event: any, issueRef: string) {
    ...
    const normalized = normalizeIssueRef(
      event?.issue ?? event?.issueRef ?? event?.identifier,
    );
  ...
  function inferLifecycleEvents(
    ...
    if (issue.lane && issue.lane !== "Unknown") {
      events.push({
        phase: "Promoted",
        label: `Promoted into ${issue.lane}`,
        time: issue.promotedAt ?? promotionTimeFromRemote(remoteEvents) ?? null,
        detail: issue.state ?? "Lane state visible.",
        url: issueUrl,
      });
    }
    ...
    if (
      ...
      !(remoteEvents ?? []).some((event) => event.phase === "Done")
    ) {
      events.push({
        phase: "Done",
        label: "Completed locally",
        time: issue.completedAt ?? null,
        detail: issue.worktree?.path
          ? "Local worktree is still present."
          : "Completion is visible in tracker readback.",
        url: issueUrl,
      });
    }
    return events;
  ...
  function hasRemotePhase(events: any[] = [], phase: string) {
    return events.some(
      (event) =>
        event.phase === phase &&
        String(event.source ?? "").startsWith("github."),
    );
  }
  ...
  function runtimeCategoryForIssue(issue: any) {
    ...
    if (issue.workerStatus === "Worker read unavailable")
  ...
  function runtimeDetailForWorker(worker: any) {
    ...
    return (
      [worker.action, worker.backend, worker.session]
        .filter(Boolean)
        .join(" · ") || "Worker is visible."
    );
  }
  ...
  function localRuntimeForWorker(worker: any, model: any) {
    return (
      [
        ...
        model?.raw?.localStatus?.head
          ? `Head ${model.raw.localStatus.head}`
          : null,
      ]
        .filter(Boolean)
        .join(" · ") || "Runtime metadata visible through lane worker readback."
    );
  }
  ...
  function localRuntimeForIssue(issue: any, model: any) {
    ...
    if (isCompletedIssue(issue))
      return local?.head
        ? `Last local head ${local.head}`
        : "Completion has no active local runtime.";
    if (issue.workerStatus === "Worker read unavailable")
      return "Session/runtime read unavailable.";
    if (local?.head)
      return `${local.head} · ${local.dirtyCount ?? 0} dirty · ${local.worktreeCount ?? 0} worktrees`;
    if (local?.issueWorktrees?.length)
      return `${local.issueWorktrees.length} local issue worktrees`;
  ...
  function workpadCategoryForIssue(issue: any) {
    ...
    if (state === "Need Human Input" || state === "Need To Clarify")
      return "Operator question thread";
  ...
  function isCompletedIssue(issue: any) {
    ...
    return (
      state === "Done" ||
      state === "Merged" ||
      /done|merged|completed|closed/i.test(
        `${issue.evidence ?? ""} ${issue.recommended ?? ""} ${issue.runtimeCategory ?? ""}`,
      )
    );
  }
  ...
  function stateToLane(state: string) {
    ...
    if (humanStates.has(normalized) || normalized === "Agent Review")
      return "Review";
    if (
      normalized === "Merging" ||
      normalized === "Done" ||
  ...
  function navigate(event: MouseEvent, href: string) {
    ...
    window.dispatchEvent(
      new CustomEvent("shea-navigate", { detail: { href } }),
    );
  }
  ...
  async function loadIssueTimeline(issue: any) {
    ...
    try {
      ...
      if (issueTimelineResponse?.available === false) {
        issueTimelineError =
          issueTimelineResponse?.error ??
          "GitHub issue comments are unavailable.";
      }
    } catch (error) {
      issueTimelineError =
  ...
  function sessionIdForIssue(issue: any) {
    const session =
      issue?.worker?.sessionId ??
      issue?.worker?.session ??
      issue?.worktree?.session ??
      issue?.session ??
      null;
    return session === "session pending" ? null : session;
  }

  function codexTranscriptLink(response: any) {
    return (
      response?.deepLink ??
  ...
  function issueSort(left: any, right: any) {
    ...
    return (
      (runtimeRank[left.runtimeCategory] ?? 9) -
        (runtimeRank[right.runtimeCategory] ?? 9) ||
...
{#if selectedIssue}
  <section class="route-hero compact">
    <div>
      <p class="eyebrow">Lane Issue View</p>
    ...
    <div class="pagination">
      ...
      <button
        class="btn btn-ghost"
        type="button"
        on:click={refreshLocalArtifacts}
        disabled={localArtifactsRefresh?.running}
      >
        {localArtifactsRefresh?.running
          ? "Refreshing local artifacts"
          : "Refresh local artifacts"}
      </button>
      <a
        class="btn btn-ghost"
        href="/lanes"
        on:click={(event) => navigate(event, "/lanes")}>Back</a
      >
    </div>
  </section>
  ...
  <section
    ...
    aria-label={`${selectedIssue.id} issue detail`}
  >
    <div class="lane-detail-summary">
      ...
      <div>
        <span class="mini-label">Last heartbeat</span>
      ...
      <div>
        ...
        <strong
          >{formatTime(
            selectedIssue.worktree?.lastProgressAt ??
              selectedIssue.worktree?.lastModified ??
      ...
      <div>
        ...
        <strong
          >{selectedIssue.worktree?.path ? "Preserved" : "Not visible"}</strong
        >
      </div>
      <div>
            ...
            <div>
              ...
              <strong
                >{formatTimeWithRelative(
                  transcriptResponse?.lastUserMessageAt,
                )}</strong
              >
            </div>
            <div>
              ...
              <strong
                >{formatTimeWithRelative(
                  transcriptResponse?.lastAssistantMessageAt,
                )}</strong
              >
            </div>
          </div>
    ...
    <div class="lane-lifecycle-list">
      ...
      {:else if issueTimelineError}
        <p class="section-note">
          GitHub issue comments unavailable: {issueTimelineError}
        </p>
      {/if}
      {#if sourceLinkError}
      ...
      {#if lifecycleEvents.length}
        {#each lifecycleEvents as event}
          <article class="lane-lifecycle-row">
            ...
            {#if event.url}
              <button
                class="queue-link source-link-button"
                type="button"
                on:click={() => openSourceLink(event.url)}>Source</button
              >
            {/if}
          </article>
        {/each}
      {:else}
        <div class="inline-empty">
          <strong>No issue timeline events visible</strong>
          <p>
            The current readback does not include timeline evidence for this
            issue.
          </p>
        </div>
      {/if}
...
{:else}
  <section
    class="lane-board-overview lane-views-board"
    aria-label="Lane issue board"
  >
    <div class="lane-board-grid">
      {#each laneColumns as lane}
        <article
          class="lane-board-column {lane.pickedCount
            ? 'success'
            : lane.issues.length
              ? 'warn'
              : 'neutral'}"
        >
          <div class="lane-board-column-head">
            ...
            {#if lane.pickedCount || lane.completedCount}
              <small
                >{lane.pickedCount} active · {lane.completedCount} completed</small
              >
            {/if}
          </div>
          ...
          <div class="lane-board-issue-list">
            {#if lane.issues.length}
              {#each lane.issues as issue}
                <a
                  class="lane-board-item {issue.runtimeTone} {issue.runtimeCategory ===
                  'Active runtime'
                    ? 'picked'
                    : ''}"
    ...
    <div class="lane-completed-head">
      <div class="lane-completed-actions">
        ...
        <div
          ...
          role="group"
          aria-label="Local worktree time window"
        >
      ...
      {#if filteredCompletedLocalIssues.length}
        <div class="lane-completed-pagination">
          <span
            >{completedPageStart}-{completedPageEnd} of {filteredCompletedLocalIssues.length}</span
          >
        ...
        {#each pagedCompletedLocalIssues as issue}
          ...
          >
            ...
            <span
              ...
              title={progressAge(issue).title}>{progressAge(issue).label}</span
            >
            <span class="lane-completed-age"
              >{relativeAge(issue.worktree?.lastModified)}</span
            >
            <span
              class:dirty={treeStateLabel(issue.worktree?.treeState) ===
                "Dirty"}
              class="lane-tree-state"
              >{treeStateLabel(issue.worktree?.treeState)}</span
            >
            <code class="lane-completed-branch"
              >{issue.worktree?.branch ?? "branch unknown"}</code
            >
            <code class="lane-completed-headsha"
              >{shortHead(issue.worktree?.head)}</code
            >
          </a>
        {/each}
      {:else}
        <div class="inline-empty compact-empty">
          ...
          <p>
            Switch to All or refresh after an issue worktree with a complete
            title is visible locally.
          </p>
        </div>
      {/if}
    </div>
