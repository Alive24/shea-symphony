...
Canonical design-system title: Shea Symphony Design System

Shea Symphony: a supervised AI-native engineering workflow cockpit for teams that run coding agents against tracker-backed work. It turns rough engineering intent into executable issue contracts, runs Main, Review, Human Review, and Merge lanes in isolated workspaces, records durable workpad and timeline evidence, and gives human operators clear review, recovery, and merge decisions.

## GitHub Repositories

- https://github.com/Alive24/shea-symphony
...
Use the supplied Starbucks-style OpenDesign template as the starting visual language for now: deep green (#006241, #00754a, #1e3932), warm cream surfaces (#faf6ee, #f2f0eb), white cards, quiet borders, rounded corners, and restrained gold accents (#cba258, #dfc49d). We will customize the palette later, so prioritize preserving the template's calm, premium, operational feel.

Shea Symphony should feel like a human operator cockpit for AI-native engineering teams, not a generic AI chat app or decorative SaaS dashboard. The product is evidence-first: issue contracts, lane claims, workpads, review ledgers, PR links, runtime status, Doctor diagnostics, and state transitions should feel like first-class UI objects.

The core workflow is: Issue Forge -> Todo issue contract -> Main lane -> Agent Review -> Human Review -> Merge lane -> Done, with Rework and Need Human Input as safe stop states. Make Main, Review, Human Review, and Merge authority boundaries visible.

Typography should be editorial but practical: strong display headings, readable body text, and precise monospace metadata for commands, claims, artifacts, timestamps, recovery keys, and dry-run/write-mode output. Avoid purple AI gradients, chat bubbles, vague automation magic, fake vanity metrics, or UI that hides the next safe operator action.
