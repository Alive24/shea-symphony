...
Status: supervised foreground Autoloop dogfood baseline with read-only GitHub
Project v2 loading through the `gh` CLI, `autopilot plan` as the mandatory
planning bridge, and bounded Autoloop (`autopilot loop`) as the normal all-lane write-mode
entrypoint. The live GitHub Project workflow now carries concise lane prompts
and a complete Markdown workpad registry, but
Shea Symphony is not a daemon or unattended background service yet.
...
| Dogfood workflow prompts and workpads | `.shea/workflows/shea-symphony.md` is the canonical normal operator workflow index/config and references `.shea/prompts/main-agent.md`, `.shea/prompts/review-agent.md`, and `.shea/prompts/merge-agent.md` plus every required file under `.shea/template/workpad/`. Main, Review, and Merge initialization load concise lane-specific prompt contracts. Workpad validation and rendering fail closed for configured missing, empty, unreadable, malformed, or unknown-variable templates instead of using embedded Rust prose. Section-aware canonical Main workpad updates preserve unrelated stable sections and append-only lane evidence. |
...
| GitHub Project v2 tracker | Fixture-backed mode plus live loading and explicit write operations through centralized `gh` helper paths. Project metadata, field metadata, REST field IDs, Status option IDs, Project item REST IDs, and supported Status/claim field updates are REST-first with process-local metadata caching and explicit GraphQL fallback for capability gaps. Rich issue/comment/linked-PR/subissue reads, Project item addition, and unsupported field mutations still use GraphQL where required. `main loop` can coordinate existing primitives, idle-poll in unbounded write mode, and use claim decision helpers before write-mode dispatch; auth diagnostics distinguish fixture mode, env-token auth, usable `gh api graphql` auth, missing `gh`, and unusable auth. Same-state status writes are skipped, Project item addition can initialize the configured `Status` field to a requested mapped state, marker workpad upsert is idempotent for the canonical marker, and claim decision helpers distinguish claimable, active, and externally changed states. Tracker mutations classify HTTP 5xx, network, and rate-limit failures as recoverable, then use read-after-write reconciliation for lane claims, workpads, timeline evidence, status writes, merge completion, and issue close; append-only evidence carries hidden idempotency markers so reruns skip duplicate lane comments. Full claim reconciliation across arbitrary stale or malformed claim values is not implemented yet. |
...
| Issue Forge | The Shea Symphony CLI exposes a compact `forge` command group: `forge validate`, `forge create`, `forge promote`, and `forge rework`. Conversation, reflection, promotion, and Human Review -> Rework revision discussion are owned by Codex skills. `forge create --status Backlog` uses a lighter Backlog seed gate, `forge create --status Todo` uses the full Issue Quality Gate, creation always adds the issue to the configured Project with the requested initial status, write success reports the tracker `issue_id` plus readback issue number, URL, and Project status when available, and live `Todo` creation requires `--assignee` before adding executable Project items. `forge promote` updates an existing Backlog issue in place with explicit dry-run and failure-checkpoint reporting, owns the structured Promotion Note comment requirement, accepts optional operator readback summaries, and keeps the Backlog-to-Todo status mutation as the final write. `forge rework` revises live `Human Review` issues into `Rework` with prepared body/evidence inputs, active-claim fail-closed diagnostic timeline comments, terminal claim preservation, timeline evidence before status mutation, and no PR/worktree requirement. Text/number/date field setup is not implemented yet. |
| Repository-owned Skills | `.agents/skills` is the only first-party Shea and HALO Skill source. Normal operational Skills are concise Workflow Capability consumers: they retain lane/authority/completion policy and defer shared mutation semantics and command syntax to the capability contract and selected adapter. The five operational Skills dropped from 1,072 to 185 lines while preserving lane boundaries. The target owns vendored copies; Shea CLI and Doctor provide no managed parity lifecycle. Structural tests validate inventory, metadata, compactness, capability references, and absence of adapter runbooks. |
...
| Setup and execution profiles | `setup-shea` is the single operator-facing onboarding and reconciliation entrypoint. It resolves one latest stable GitHub Release to a full immutable commit, vendors selected project-local Skills through the standard Skills CLI, preserves target-owned customizations, binds repository contracts, and proposes a confirmed credential-free `.shea/runtime-profile.json`. Generic core validation checks repository identity, Git-blob drift, resolved executable versions, and bounded environment data. Main prepares/reuses the exact worktree and passes runtime readiness before claim, re-reads tracker ownership immediately before claim, and applies the same runtime overlay to backend execution and handoff verification. Setup never claims an issue or launches a lane. |
| Agent backends | Dry-run, Codex app-server, Claude Code stream-json, and tmux fallback backends exist. The canonical Main runtime defaults to Codex app-server; operators may select `claude-code` for Main, independent Review, and semantic Merge-agent repair through one structured transport. Both structured transports record prompt, raw protocol, stderr, normalized events, exit status, runtime state, and session evidence. Claude Main recovery validates issue, lane, run, worktree, and session identity before `--resume`; Claude Review starts fresh and may resume only its own interrupted job. Model, authentication, gateway, environment, and permission policy remain in the configured Claude command or wrapper. Autoloop is the foreground controller rather than a backend. Clean merge remains direct CLI behavior and never launches Claude. Dynamic-tool execution remains a follow-up. |
...
| Agent Review | Finding classes, fake reviewer lifecycle, native schema-constrained `agy` CLI review support, legacy Gemini CLI fallback support, and independent Codex app-server Review support exist. agy capability checks are version-agnostic; its provider envelope and structured result are parsed separately, with malformed, missing, conflicting, non-success, or zero-exit-invalid output failing closed and remaining diagnostic evidence preserved. Codex Review starts a fresh read-only/non-interactive thread per job, permits only a same-job interrupted resume, validates structured reports, preserves provider/protocol evidence, and fails closed on interactive, malformed, missing, timed-out, cancelled, or workspace-mutating runs. All backends reuse the same role-bound transition decisions, evidence-first Rework diagnostics, invalid-handoff checks, parent/subissue-aware routing, bounded worker pool, Project claim markers, health classification, durable job ledger, and workpad/status evidence. Persistent background review worker supervision is not implemented yet. |
| Merging | `merge once` can inspect issues already in `Merging`, require one linked PR, treat Project `Merging` as the approval signal, derive fixture merge preflight status from linked PR metadata for dry-run and write-mode rehearsal, check live GitHub PR state/review/check/mergeability data where available, validate PR base branches from parent/subissue topology evidence, recheck transient missing or `UNKNOWN` mergeability once before deciding, merge clean PRs only with explicit `--write`, record a `Shea Symphony Merge Run` timeline comment, set Project `Done`, close the linked GitHub issue when supported by the tracker, safely update `BEHIND` PR branches with `gh pr update-branch` and leave them in `Merging` for retry, leave transient pending/unknown cases in `Merging`, attempt `DIRTY` direct mechanical conflict repair in a clean local PR worktree, abort interrupted conflict-repair merge states before retry, route mechanical content conflicts through the configured merge-agent backend before escalation, keep successful mechanical or merge-agent repair in `Merging` for retry, keep retryable backend or verification failures in `Merging` after cleanup, and route only semantic uncertainty, unsafe/untrusted preconditions, untracked-file residue, push-failing, or checks-failing blockers to `Need Human Input` with a concrete timeline question instead of defaulting to `Rework`. If `gh pr merge` fails with a recoverable transport/rate-limit class, merge readback checks whether the PR is already `MERGED` and continues Project `Done` / issue-close reconciliation when proven. Bounded `merge loop` can repeat that guarded tick for an explicit iteration count, `--max-concurrent N` can process multiple guarded merge slots while skipping issues claimed by another `Merging Agent`, and write mode adopts interrupted structured merge-loop/goal claims before continuing normal selection unless `--no-recover` is set. Fixture workflows cover controlled successful merge and conflict-repair rehearsals without live GitHub mutation. Unbounded continuous merge polling is not implemented yet. |
| Project doctor | Read-only `doctor` / `audit-project` reports workflow invariant violations from normalized tracker issues, including missing PR handoff evidence, Agent Review draft PRs, missing review pass evidence, dirty Merging PRs, ambiguous issue worktree candidates, missing runtime ownership hints, queued issues with PRs, stale registered runtime sessions, orphaned/unattributed session registry entries, session/runtime mismatches, sessions needing operator attention, and native parent/subissue topology diagnostics such as unsafe parent integration branch evidence, subissue PR targets, subissue `Done` merge evidence, and premature parent `Human Review`. Doctor includes backend-aware wording for app-server, tmux fallback, and manual Codex App session evidence, and evaluates multiple Main runtime entries per issue. Active structured claims without matching registry evidence stay prominent warnings, while terminal structured claims without registry evidence use lower-noise historical guidance. Project Status `Done` issues are skipped for issue-internal operational checks, but tracker terminal-state mismatches still report. Human Review pass evidence can come from Project fields or from an Agent Review timeline comment. JSON output and strict blocker failure modes exist. `doctor repair <issue> --mark-pr-ready --confirm-handoff-ready --write` can repair draft PR handoffs only with explicit operator confirmation, and `doctor-repair-human-review` can repair the specific invalid Human Review-without-pass-evidence case with explicit `--write`. The repo-owned Doctor skill at `.agents/skills/shea-symphony-doctor/SKILL.md` defines read-first triage and bounded repository-contract repair; it does not manage vendored Skills or enforce upstream equality. |
| Observability | Operator-readable terminal snapshots default to real lane/session work, retry/skipped issue sections, gate details, latest issue-scoped status bars, dry-run cleanup candidates, and recent registered runtime session summaries with backend, status, evidence source, attach command, artifact, and log path when available. Runtime telemetry such as polling cadence, token counters, and the event-log path remains in `status show --json` and app issue details instead of the default human log stream. The read-only `debug` command composes Project, doctor, smoke readiness, runtime/session, cleanup, and lane next-action signals into one human report without claiming work or implying unattended readiness. `main loop`, `review loop`, and `merge once` emit compact issue-scoped `Latest:` lines for current lane/action/category while no-issue idle status stays debug-only. Long-running tracker and lane/backend waits now emit compact stderr `progress ...` heartbeats after the centralized threshold, with JSON stdout kept clean and lane-loop heartbeat records appended to the local event log when available. `autopilot loop`, `main loop`, `project state`, and `doctor` support opt-in `--display tui` panel views backed by the Codex-aligned `ratatui` / `crossterm` stack while preserving default line output for logs and scripts; the Autoloop dashboard is rendered from a shared snapshot shape intended to be reusable by a future Web UI instead of terminal-string parsing. `status show WORKFLOW --json` prints a local-only `RuntimeSnapshot` shape with session registry status and the event-log path; it does not read GitHub Project, and Project-backed planning remains owned by `autopilot plan`. `status serve WORKFLOW --once` serves that local snapshot on local `GET /status.json` / `GET /status` for one request. JSONL event-log primitives exist, can read back records, and can produce compact summaries by event, issue, and session; `main once` writes dry-run events with actor and profile metadata. `tracker_mutation` audit records now capture logical actor role/label, command, mutation type, issue, target, from/to state when known, reason, and timestamp for state changes, claim field writes, workpad/timeline writes, PR links, issue/project creation, review routing, merge routing, and repair actions. `TokenTotals::from_agent_events` provides a first aggregation helper for normalized token events. Runtime state files are written during write-mode `main loop` issue execution, including actor role/label, git author, optional profile/instance identity, app-server artifact pointers, resume, retry, usage-limit pause, stall supervision events, and session registry probes. Repo-owned live implementation and review workflows default durable roots under `~/.shea-symphony/artifacts`, support `SHEA_SYMPHONY_ARTIFACT_ROOT`, and warn when an operator points commands at temp-only workflow files. Persistent/remote web/API service mode is not implemented yet. |
| Usage-limit pause/resume | Conservative usage-limit/rate-limit/resource-exhausted classification exists for subprocess agent events and review job output. `main loop` records usage-limit pauses in workpad/runtime retry state and does not advance to `Agent Review`; backend-backed `review loop` records quota/rate-limit health evidence, honors detected retry-after windows when available, and waits or backs off without moving the issue to `Human Review`. Vendor-specific quota management is not implemented. |
| Tests | Unit tests cover the dry-run skeleton. Read-only / dry-run live GitHub smoke tests exist behind explicit `SHEA_LIVE_GITHUB_SMOKE=1` opt-in; mutation and Linear credential-gated smoke coverage are still missing. |
...
directly. `autopilot plan` is read-only and must run before write-mode
Autoloop. `autopilot loop --write` is the normal bounded foreground all-lane
...
Autoloop lane throughput is independent: the foreground supervisor lifetime is
...
## Native tmux Fallback Supervision Contract

Shea Symphony's tmux layer is now an operator-supervised fallback/debug
terminal substrate owned by the Shea Symphony runtime, not a second workflow
engine and not the default Main runtime. The tracker remains authoritative for
issue lifecycle, while the session registry and logs provide durable local
...
1. Start normal supervised all-lane dogfood with `autopilot plan`, then a
   bounded `autopilot loop --write`. Use `main loop --write` only when
   intentionally isolating Main, or manually claim with `main claim`,
   `review claim`, or `merge claim`. Manual claims record non-tmux registry
   evidence for the printed `run=`; start runtime with
   `session start --lane main|review|merge --run <RUN_ID>` only when a
   supervised fallback session is needed.
...
4. Use `clean audit` to classify registry files, rendered prompts, app-server
   artifacts, tmux logs, session records, and terminal worktrees before any
...
These must exist before Shea Symphony can safely dogfood against real GitHub
Project v2 issues:
...
2. Harden GitHub tracker writes.
   ...
   - PR relationship verification is first-class for lane transitions:
     ...
     `Closes #<issue>` relationship, and if Shea Symphony can identify a PR but
...
- `forge validate --issue` and read-only `project inspect` expose
  ...
  remains a lane decision that must write lane evidence before changing
  state.
...
4. Agent Review authority boundary.
   ...
   - Independent Review Agent commands can set `Human Review` only after a
     passed review with evidence on ordinary issues or parent final issues;
     routine native subissue pass evidence routes to `Merging`.
   - Manual/operator review uses `review claim`, `review pass`, and
     ...
     the doctor-recognized review pass marker before routing to `Human Review`
     or, for routine native subissues, `Merging`;
   ...
   - Terminal review backend failures and timeouts are classified before
     routing: quota/rate-limit/resource-exhausted responses wait and retry,
     transient capacity/network/timeout failures back off, and command/auth,
     model, policy, or tool-permission failures require `Need Human Input`.
     Same-cause repeated failures write compact repeat evidence, and recoverable
     cases preserve `Agent Review` rather than stopping the review loop.
   - Write-mode review jobs persist a JSON ledger record under the configured
     ...
     provider session identity, and finding count; review and Rework workpads
     link to that ledger when available. Codex app-server artifacts additionally
     point to raw protocol, stderr, normalized events, thread identity, resume
     count, and workspace-integrity evidence.
...
6. Live agent execution.
   - The live workflow loads concise repository Markdown lane prompts that name
     authority, capabilities, and stop conditions without embedding issue bodies
     or a second command runbook.
   - Keep prompt and workpad prose in `.shea/prompts/` and
     `.shea/template/workpad/`; runtime code owns mechanics rather than prose.
...
8. Observability.
   ...
   - `doctor` / `audit-project` is available as a project/workflow/runtime
     ...
     accepted from explicit Project fields when available or from an Agent
     Review timeline marker `Review pass evidence: recorded`; a `Review Agent`
     ...
     writing a Doctor timeline comment. Native parent/subissue topology diagnostics are
...
9. Integration profile.
   ...
   - cockpit-tools integration remains a small adapter over the local Codex
     ...
     and account binding fields. Shea Symphony reads only non-secret instance identity
...
- records Claude and Shea session identity for Main, independent Review, and semantic Merge repair.
- rejects malformed, truncated, cancelled, timed-out, and unexpected-exit runs.
- validates Claude Review reports and rejects any reviewed-workspace mutation.
...
- passed ordinary and parent-final reviews move to `Human Review`; passed
  routine native subissue reviews move to `Merging`; confirmed findings and
  completed but inconclusive automatic reviews move to `Rework`; failed, timed
  out, or unavailable reviews must not set `Human Review`.
...
- records stale reason, rework class, prior/current head SHA, prior/current base
  SHA, changed files, and patch summary in merge/rework timeline evidence.
...
`merge claim` may contain spaces. Shea Symphony stores those labels with
