...
| Repository-owned Skills | `.agents/skills` is the only first-party Shea and HALO Skill source. The standard Skills CLI can discover that tree and vendor selected Skills into a target repository once; the target then owns and may customize those files. Shea CLI and Doctor provide no install, update, remove, restore, version, manifest, or upstream-text parity lifecycle. Structural tests validate the canonical inventory, frontmatter, metadata, and referenced resources directly under `.agents/skills`. |
