...
| Workspace | Local path sanitization, creation, timeout-aware hooks, stdout/stderr capture, `before_remove`, safe cleanup helpers, repository-local git identity application, workspace/branch/PR handoff planning, live git worktree/branch creation, branch push, PR create-or-reuse, run-loop handoff evidence, profile-scoped workspace keys, a namespaced artifact layout, dry-run cleanup planning, and an Agent Review handoff invariant that blocks missing PR evidence before `Agent Review` exist. Write-mode runtime reconciliation cleanup is not wired yet. |
...
7. Long-running orchestration.
   ...
   - `cleanup-plan` can report terminal worktree candidates without deleting
     files. Remaining work: explicit write-mode cleanup after operator review.
...
- terminal workspace cleanup write-mode wiring in the runtime loop.
