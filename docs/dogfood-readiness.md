...
entrypoint. The live GitHub Project workflow now carries concise lane/backend
prompts and a manifest-selected Markdown resource closure, but
...
| Installable resource contract | `.shea/resources.v1.json` declares one always-enabled core group and explicit Improve, HALO research, parent/subissue, and future shea-docs extensions. Runtime/setup resolution validates the selected dependency closure, excludes global `setup-shea`, and reports exact Markdown sources without creating target package state. |
| Dogfood workflow prompts and templates | `.shea/workflows/shea-symphony.md` references concise Main, Review, and Merge lane prompts plus Markdown backend fragments for Codex app-server, automatic/structured Review, Claude Code Review, and Merge repair. Templates are classified under `template/workpad`, `template/evidence`, `template/decision`, and `template/report`; enabled closure validation replaces the fixed all-template bundle. Rust owns typed facts and protocol enforcement, not fallback behavior prose. |
...
| Setup and execution profiles | `setup-shea` is the global operator-facing onboarding and reconciliation entrypoint. It resolves one latest stable GitHub Release to a full immutable commit, loads the source manifest, installs the complete core group plus explicitly selected extensions, vendors Skills through the standard Skills CLI, preserves target-owned customizations, and proposes a confirmed credential-free `.shea/runtime-profile.json`. Setup never vendors itself, claims an issue, or launches a lane. |
...
6. Live agent execution.
   ...
   - Keep behavioral prose in `.shea/prompts/` and the classified
     `.shea/template/` taxonomy; runtime code owns typed facts, validation, and
     protocol mechanics rather than prose.
