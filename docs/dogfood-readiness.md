...
| Local skill suite | Repo-owned Shea Symphony skills live under `skills/shea-symphony/` with dated release metadata and an installable `setup-shea` entrypoint. It uses the standard Skills CLI for selectable project-local Codex, Claude Code, and Antigravity surfaces, plus a checksum- and runtime-identity-verified standalone Legacy CLI. The normal set includes Setup, Runtime Onboarding, Doctor, Issue Forge, Investigate, Reflect, Manual Main, Manual Review, Human Review, and Manual Merge; Dream and HALO research remain explicit additions. `setup-shea` discovers repository/runtime/Project requirements, previews exact local and external actions, gates them under separate confirmations, reconciles managed `.shea` contracts without overwriting operator edits, and finishes with no-claim readiness. `skills status` and `doctor` remain read-only diagnostics; confirmed install/update writes belong to `setup-shea`. |
