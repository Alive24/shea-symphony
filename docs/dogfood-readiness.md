...
| Execution profiles | Workflow/Cockpit profiles still select backend identity and workspace namespaces. Separately, the repository runtime-onboarding skill proposes a confirmed, credential-free `.shea/runtime-profile.json`; generic core validation checks repository identity, Git-blob drift, resolved executable versions, and bounded environment data. Main prepares/reuses the exact worktree and passes runtime readiness before claim, re-reads tracker ownership immediately before claim, and applies the same runtime overlay to backend execution and handoff verification. Missing optional profiles preserve legacy behavior; adopted targets can require the profile. Review and Merge do not consume it in this slice. |
