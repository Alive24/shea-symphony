...
| Agent backends | Dry-run, Codex app-server, tmux fallback, and conservative Claude Code subprocess backends exist. The configured workflow selects the Main backend; `autopilot loop` is not an app-server, tmux session, or backend process by itself. It invokes the configured lane commands and preserves each lane runtime boundary. Manual break-glass recovery still claims with `main claim`, `review claim`, or `merge claim` before optional `session start --run <RUN_ID>`. Clean `merge once` / `merge loop` remains direct CLI merge behavior and does not launch a merge-agent runtime. Dynamic-tool execution and Claude Code protocol parity remain follow-ups. |
