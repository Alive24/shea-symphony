...
| Agent Review | Finding classes, fake reviewer lifecycle, headless `agy` CLI review support, legacy Gemini CLI fallback support, independent Codex app-server Review, and independent Claude Code Review support exist. Codex and Claude Review start fresh read-only/non-interactive sessions per job, permit only a same-job interrupted resume, validate structured reports, preserve provider/protocol evidence, and fail closed on malformed, missing, timed-out, cancelled, or workspace-mutating runs. Claude Review reuses the shared stream-json transport without changing the existing report, job, ledger, worker-pool, or routing boundaries. All backends reuse the same role-bound transition decisions, evidence-first Rework diagnostics, invalid-handoff checks, parent/subissue-aware routing, bounded worker pool, Project claim markers, health classification, durable job ledger, and workpad/status evidence. Persistent background review worker supervision is not implemented yet. |
