...
Status: supervised foreground autopilot dogfood baseline with read-only GitHub
Project v2 loading through the `gh` CLI, `autopilot plan` as the mandatory
planning bridge, and bounded `autopilot loop` as the normal all-lane write-mode
...
4. Agent Review authority boundary.
   ...
   - Terminal Gemini review backend failures and timeouts are classified before
     ...
     model, policy, or allowed-tools failures require `Need Human Input`.
