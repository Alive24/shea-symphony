...
| --- | --- | --- | --- | --- |
| Full Codex app-server stdio protocol | `SPEC.md`, `elixir/lib/symphony_elixir/codex/app_server.ex` | Partial | A conservative workspace-bound Codex subprocess backend exists, and configured `codex app-server` commands now route through a minimal local stdio JSON-RPC transport that sends initialize/thread/start/turn messages, normalizes core runtime events, and records durable prompt/protocol/stderr/event artifacts. Continuation turns, dynamic tools, remote launch, full runtime status integration, and live Codex validation still require follow-up work. | Use the app-server transport as the Main/Merge runtime substrate, then add runtime/status/Doctor wiring, dynamic-tool protocol support when needed, and live smoke coverage. |
| Strict Liquid-compatible prompt and workpad engine | `SPEC.md`, `elixir/lib/symphony_elixir/prompt_builder.ex` | Implemented | Prompt and workpad templates render through the Rust `liquid` engine with standard tags and filters, plus strict external-variable validation and validation-time smoke rendering. | Keep conformance coverage current as configurable prompt/workpad templates gain new context values. |
