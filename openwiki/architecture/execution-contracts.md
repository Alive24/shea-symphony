...
This is the **Draft target flow**. Currently `IssueWorkflow` schedules only `NoopCoreActivity`; agent Activities return `not_implemented`; `TrackerTransitionActivity` is inert; and the Workflow does not schedule local projection (`src/symphony/workflows.rs`, `activities.rs`). **Tracking (verified 2026-07-28):** #494 is Done only for the transition DTO/idempotency contract. Remaining transition work has no promoted T2607-04 Issue; agent and Workflow behavior belongs to T2607-05/06, also with no promoted Issues.
...
**Checked-in implementation.** No lease DTO, lease Activity, lease liveness, or capability-to-lease enforcement exists in the inspected `src/symphony/workflows.rs`, `activities.rs`, or `dto.rs`. Agent placeholders do not touch worktrees. **Tracking:** T2607-05 owns lease/capability enforcement; no Issue is promoted.
...
**Checked-in implementation.** The no-op Workflow sets only a 30-second `start_to_close` timeout. There are no heartbeat DTOs or calls, layered timeout policy, progress projection calls, or no-progress detection; these belong to T2607-05/06, with no promoted Issues. `workers.rs` reports configured per-queue concurrency but the worker builders do not visibly apply those values. **Tracking:** enforcement is an unowned T2607-01 residual that must be settled before T2607-05/06 rely on it.
...
**Checked-in implementation.** The Workflow defines no Signal or Update handlers and schedules no real child backend. Agent placeholders implement no cancellation request, confirmation, or worktree-safety result. **Tracking:** T2607-05/06 own Activity cancellation and Workflow handlers; no Issues are promoted.
...
**Checked-in implementation.** The module comments enforce the architectural separation, and the transition request/result DTO is compact and history-safe. However, no checked-in agent Activity performs product work, no Workflow calls the transition Activity, the transition Activity performs no tracker I/O, and local projection Activities are placeholders. The prohibition is an implementation requirement, not proof that the planned end-to-end authority path exists. **Tracking:** #494 is Done only for transition DTO/idempotency; remaining transition/projection work is unpromoted T2607-04, while agent and Workflow integration is unpromoted T2607-05/06.
...
3. **Re-express proven 2606 behavior through coarse Activities.** Use the [protected 2606 runtime](2606-bootstrap-runtime.md) as a behavior/test/evidence acceptance oracle, then implement those semantics through new typed Activity, capability, lease, and adapter contracts. Bounded protocol-independent DTOs, parsers, tracker/Git adapters, event normalization, helpers, and focused tests may be reused after ownership review when they fit those boundaries. Do not call, embed, or wrap the old product CLI, lane loops, protected-runtime ownership, hidden state authority, or legacy orchestration beneath Temporal.
...
The corrected Draft inputs require the orchestration and authority contracts to be implemented as new 2607 code. `RUNTIME-ROLE-MAPPING.md` and `CODE-OWNERSHIP-MAP.md` classify 2606 behavior, tests, protocols, and operational evidence as inputs for new typed contracts—not reusable orchestration/runtime substrate—while permitting reviewed reuse of bounded protocol-independent components and focused tests that fit the new ownership boundary. `APP-CLI-SPLIT.md` likewise requires any thin admin/debug wrapper to be new code over new Temporal APIs rather than a retained or rewired 2606 command graph.
