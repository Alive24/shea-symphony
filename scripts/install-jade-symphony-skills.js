...
const defaultSuiteDir = path.join(repoRoot, "skills", "jade-symphony");
const ignoredFileNames = new Set([".DS_Store"]);

function usage() {
  return `Usage: node scripts/install-jade-symphony-skills.js [options]

Install or validate the repo-owned Jade Symphony skill suite.
...
function printPlan(manifest, skills, selectedTargets, mode) {
  console.log(`Jade Symphony skill suite ${manifest.version} (${manifest.releaseDate})`);
