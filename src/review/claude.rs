...
case "$SHEA_TEST_MODE" in
  ...
  missing-report)
    ...
    ;;
  contradictory)
    printf '%s%s%s\n' '{"type":"result","subtype":"success","is_error":false,"session_id":"' "$session_id" '","result":"{\"summary\":\"Contradictory pass.\",\"terminal_classification\":\"pass\",\"findings\":[{\"class\":\"confirmed\",\"severity\":\"high\",\"title\":\"Defect\",\"body\":\"A confirmed defect exists.\",\"file\":\"src/lib.rs\",\"line\":7,\"evidence\":\"fixture evidence\"}]}"}'
    ;;
  incomplete)
    printf '%s%s%s\n' '{"type":"result","subtype":"success","is_error":false,"session_id":"' "$session_id" '","result":"{\"summary\":\"Incomplete finding.\",\"terminal_classification\":\"rework\",\"findings\":[{\"class\":\"confirmed\",\"severity\":\"high\",\"title\":\"Defect\",\"body\":\"Missing required evidence.\",\"file\":\"src/lib.rs\",\"line\":7}]}"}'
    ;;
  unsupported-severity)
    printf '%s%s%s\n' '{"type":"result","subtype":"success","is_error":false,"session_id":"' "$session_id" '","result":"{\"summary\":\"Invalid severity.\",\"terminal_classification\":\"rework\",\"findings\":[{\"class\":\"confirmed\",\"severity\":\"urgent\",\"title\":\"Defect\",\"body\":\"Severity is outside the contract.\",\"file\":\"src/lib.rs\",\"line\":7,\"evidence\":\"fixture evidence\"}]}"}'
    ;;
  error)
...
"#;
    ...
    fn new_jobs_are_fresh_and_parallel_artifacts_and_sessions_are_isolated() {
        ...
        let first_ledger =
            super::super::write_review_job_ledger_record(temp.path(), &issue("#518"), &first)
                .unwrap();
        let second_ledger =
            super::super::write_review_job_ledger_record(temp.path(), &issue("#518"), &second)
                .unwrap();
        assert_ne!(first_ledger, second_ledger);
        let first_record: super::super::ReviewJobLedgerRecord =
            serde_json::from_slice(&fs::read(first_ledger).unwrap()).unwrap();
        let second_record: super::super::ReviewJobLedgerRecord =
            serde_json::from_slice(&fs::read(second_ledger).unwrap()).unwrap();
        assert_ne!(first_record.job_id, second_record.job_id);
        assert_ne!(
            first_record.backend_session_id,
            second_record.backend_session_id
        );
    ...
    #[test]
    fn invalid_output_failure_timeout_cancel_and_mutation_fail_closed() {
        for mode in [
            "malformed",
            "truncated",
            "missing-report",
    ...
    fn claude_review_live_local_read_only_uat() {
        ...
        let workflow_path =
            Path::new(env!("CARGO_MANIFEST_DIR")).join("workflows/shea-symphony.md");
