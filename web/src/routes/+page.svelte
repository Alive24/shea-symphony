<script lang="ts">
  ...
  $: currentWorkerCount = Object.values(view.laneWorkers ?? {}).reduce<number>(
    (sum, workers: any) => sum + (Array.isArray(workers) ? workers.length : 0),
    0
  );
  ...
  $: laneBoard = ['main', 'review', 'merge'].map((laneKey) => {
    ...
    const workerIssues = workers.map((worker, index) => ({
      kind: 'picked',
    ...
    return {
      ...
      pickedCount: workers.length,
      queuedCount: waitingIssues.length,
      tone: workers.length ? 'success' : waitingIssues.length ? 'warn' : 'neutral'
    };
  });
