...
- `agy`-backed `review loop` runs headlessly by default with `agy --print` prompt transport and durable stdout/stderr/job-ledger evidence. Treat automatic headless review as report-only: the Shea Symphony CLI wrapper will record evidence and change state after the backend process exits.
